...
- Distributed ordering and correctness MUST NOT rely on synchronized clocks;
  fencing and version semantics belong to `go-lease` and related packages.
...
- Standardize clock seams in `go-cache`, `go-circuit-breaker`,
  `go-authentication`, `go-authorization`, `go-http-client`, `go-idempotency`,
  `go-lease`, `go-rate-limit`, `go-scheduler`, and `go-service`.
- Packages that only need timestamps consume `Clock`, not timer APIs.
- `go-calendar` consumes wall-time values but owns civil-calendar arithmetic.
- `go-temporal` consumes timestamps and durations but owns interval algebra.
